...
ACCURACY_MODE=$(python3 "$CONFIG_PY" --get --key accuracy_mode_enabled | tail -n 1 | awk -F' = ' '{print $2}' | tr -d '[:space:]')
...
if [ -z "$ACCURACY_MODE" ]; then
    python3 "$CONFIG_PY" --key accuracy_mode_enabled --value 0
    ACCURACY_MODE=0
fi
...
if [ "$IS_DEVELOPMENT" = "1" ]; then
    ...
    # Start spool services if accuracy mode is enabled
    if [ "$ACCURACY_MODE" = "1" ]; then
        echo "[INFO] Accuracy Mode enabled - starting spool services..."
