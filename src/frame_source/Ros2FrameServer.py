...
class FrameServer(Node, FrameSource):
    ...
    def _frame_metadata_callback(self, msg):
        ...
        SINGLE SOURCE OF TRUTH: frame_index from FrameMetadata
        This receives the canonical frame ID assigned by SpoolProcessor along with
        full metadata (seq, session_id, etc.). We only need frame_index for correlation.
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"[Ros2FrameServer] Received frame_index from metadata: {spool_frame_idx}, "
                           f"queue_size={self._pending_frame_indices.qsize()}")
            except queue.Full:
                ...
                try:
                    ...
                    logger.warning(f"[Ros2FrameServer] Pending index queue full, dropped frame_index {dropped_idx}")
