...
def find_start_codes(data: bytes) -> List[Tuple[int, int]]:
    """
    Find all Annex B start codes in the data.
    
    Args:
        data: H.264 Annex B encoded data
        
    Returns:
        List of tuples (offset, start_code_length) where offset is the
        byte position immediately after the start code.
        
    Note:
        Start codes are either 0x000001 (3 bytes) or 0x00000001 (4 bytes).
        The 4-byte version is typically used at access unit boundaries.
    """
    start_codes = []
    i = 0
    data_len = len(data)
    ...
    while i < data_len - 2:
        ...
        if data[i] == 0 and data[i + 1] == 0 and data[i + 2] == 1:
            ...
            if i > 0 and data[i - 1] == 0:
                # Already counted as part of 4-byte start code
                i += 1
                continue
            start_codes.append((i + 3, 3))
            i += 3
        elif i < data_len - 3 and data[i] == 0 and data[i + 1] == 0 and data[i + 2] == 0 and data[i + 3] == 1:
            start_codes.append((i + 4, 4))
            i += 4
        else:
            i += 1
...
def get_nal_unit_at_offset(data: bytes, offset: int) -> Optional[NALUnit]:
    """
    Get the NAL unit at a specific offset (useful for debugging).
    
    Args:
        data: H.264 Annex B encoded data
        offset: Byte offset in the data
        
    Returns:
        NALUnit at that offset, or None if not found
    """
    nal_units = parse_nal_units(data)
    
    for nal in nal_units:
        nal_start = nal.offset - nal.start_code_len
        nal_end = nal.offset + nal.length
        if nal_start <= offset < nal_end:
            return nal
